...
#         self.conv1 = GCNConv(num_node_features*2, 64*2)
#         self.conv2 = GCNConv(64*2, 64*2)
...
#         self.lin_text = torch.nn.Linear(64, num_node_features)# 和node_feature维度一样
        self.lin_text = torch.nn.Linear(64*2, 64)# pair 到 单个
#         self.lin_x_text = torch.nn.Linear(64*4, 64*2)# pair 到 单个
...
#         print(nodenum)
        imgpos = imgpos.cuda()
#         print(x.size()) # torch.Size([211, 8])
        x = self.conv1(x, edge_index)
        x = F.relu(x)
        #x = F.dropout(x, training=self.training)
        x = self.conv2(x, edge_index)
        x = F.relu(x)
#         print(x.size()) # torch.Size([211, 64])
...
#         print(textout.size()) # torch.Size([211, 64])
#         textout = self.lin_text(textout)
    
#         x_text_cat =torch.cat((x,textout),dim=1)
# #         print(x_text_cat.size())
#         x_text_cat = self.conv1(x_text_cat, edge_index)
#         x_text_cat = F.relu(x_text_cat)
#         x_text_cat = self.conv2(x_text_cat, edge_index)
#         x_text_cat = F.relu(x_text_cat)
# #         print(x_text_cat.size())  # 
#         # combine text node feature
#         x1x_text = x_text_cat[edge_index[0]]
#         x1x_text = x_text_cat[edge_index[1]]
#         x_text_pair =torch.cat((x1x_text,x1x_text),dim=1)
# #         print(x_text_pair.size())
#         x_text_pair = F.relu(self.lin_x_text(x_text_pair))
